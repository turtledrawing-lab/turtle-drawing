...
## 📥 스케치업 파일 완벽하게 불러오기 (재질 포함)

스케치업 모델을 **재질(텍스처)까지 그대로** 가져오려면 OBJ 로 내보낼 때 재질을 함께 내보내고, 폴더 통째로 불러오면 돼요.

### 1. 스케치업에서 OBJ 내보내기
1. **File → Export → 3D Model...**
2. 파일 형식을 **OBJ File (\*.obj)** 으로 선택
3. **Options...** 버튼 → **Export Texture Maps** 체크 ✅ (재질을 같이 내보내는 핵심 옵션)
4. 내보내기 → 한 폴더 안에 다음이 생성됩니다:
   - `모델이름.obj` — 형상(메쉬)
   - `모델이름.mtl` — 재질 정의
   - 텍스처 이미지 폴더 (`.jpg` / `.png` 등)

### 2. Turtle Drawing 으로 불러오기
- **`.obj`, `.mtl`, 텍스처 이미지가 모두 같은 폴더 안에 있어야** 재질이 정상적으로 연결돼요.
- 메뉴 → **File → Import → OBJ** 에서 그 **폴더(또는 `.obj` 파일)** 를 선택해 불러오세요.
- `.mtl` 과 텍스처가 같은 위치에 있으면 재질·텍스처가 자동으로 매핑됩니다.

> 💡 **팁**: 재질이 안 보인다면 `.obj` 파일만 따로 옮기지 않았는지 확인하세요. `.obj` / `.mtl` / 텍스처 폴더는 항상 **함께** 두어야 합니다. 스케치업에서 그룹/컴포넌트로 묶여 있던 구조는 불러올 때 블록으로 유지됩니다.
